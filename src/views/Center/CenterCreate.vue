    ...
    <div class="container mt-3">
      ...
      <div class="mb-3 mt-3">
        ...
        <div class="input-group">
          <input type="number" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" placeholder="일반가를 입력하세요." autocomplete="off" v-model="norPrice" required step="100"/>
      ...
      <div class="mb-3">
        ...
        <div class="input-group">
          <input type="number" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" placeholder="회원가를 입력하세요." autocomplete="off" v-model="memPrice" required step="100"/>
          <span class="input-group-text">원</span>
        </div>
      </div>
      <div class="d-flex">
        <div class="p-2 flex-fill d-grid">
          <button type="button" class="btn btn-primary" @click="addCenter">생성</button>
        </div>
        <div class="p-2 flex-fill d-grid">
...
export default {
  ...
  methods: {
    ...
    addCenter() {
      ...
      console.log(this.centerTel);
      axios
        .post('http://175.197.92.213:11000/center', {
